...
class Categoriser(categories: java.util.List[Category]) {
  ...
  def categorise(sentence: String): java.util.List[CategoryMatch] = {
    categories.toList.map(c => CategoryMatch(c.name, matchCategory(sentence, c))).filter(_.entryMatches.length > 0)
  }

  private def matchCategory(sentence: String, category: Category): java.util.List[EntryMatch] = {
    matchers(category).map(p => EntryMatch(p.label, p.matcher.name, p.exactEntry, p.matcher.doMatch(sentence, p.exactEntry))).filter(_.matches.length > 0)
  }
}